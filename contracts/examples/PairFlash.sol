...
/// @notice An example contract using the Panaromaswap V3 flash function
contract PairFlash is IPanaromaswapV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `panaromaswapV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IPanaromaswapV3Pool pool = IPanaromaswapV3Pool(PoolAddress.computeAddress(factory, poolKey));
